...
function QuickCreateModal({ type, open, onClose }: QuickCreateModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (type === 'service') {
      addService({
        id: `svc-${Date.now()}`,
        name: form.title || 'Servicio nuevo',
        duration: Number(form.duration) || 45,
        price: Number(form.price) || 1200,
        description: form.notes || 'Servicio agregado desde el modal de creación rápida.',
        category: 'General',
      })
    }
    ...
    if (type === 'branch') {
      addTenant({
        id: `tenant-${Date.now()}`,
        name: form.title || 'Nueva sucursal',
        city: form.city || 'Ciudad de México',
        speciality: 'Atención profesional',
        address: form.address || 'Dirección nueva',
        active: true,
        calendarLinked: true,
      })
    }
  ...
  return (
    <Modal
      ...
      title={
        type === 'appointment' ? 'Nueva cita' : type === 'service' ? 'Agregar servicio' : 'Crear sucursal'
      }
      subtitle="Completa los datos para añadirlo al sistema de forma inmediata."
    >
      <div className="max-h-[70vh] space-y-3 overflow-y-auto px-[3px]">
              ...
              <div className={isDark ? 'space-y-2 rounded-2xl border border-slate-700 bg-slate-950/60 p-3' : 'space-y-2 rounded-2xl border border-slate-300 bg-slate-50 p-3'}>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className={isDark ? 'rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs uppercase tracking-[0.18em] text-emerald-200' : 'rounded-xl border border-emerald-300 bg-emerald-100 px-3 py-2 text-xs uppercase tracking-[0.18em] text-emerald-700'}
                  >
                    Guardar cliente
                  </button>
                </div>
        ...
        {type === 'branch' && (
          <>
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Nombre de la sucursal" className={isDark ? 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-600 bg-slate-900/70 p-3 text-sm text-white transition focus:border-emerald-500/60 focus:outline-none focus:ring-2 focus:ring-emerald-500/20' : 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-300 bg-white p-3 text-sm text-slate-900 transition focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200'} />
            <div className="grid gap-3 sm:grid-cols-2">
              <input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="Ciudad" className={isDark ? 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-600 bg-slate-900/70 p-3 text-sm text-white transition focus:border-emerald-500/60 focus:outline-none focus:ring-2 focus:ring-emerald-500/20' : 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-300 bg-white p-3 text-sm text-slate-900 transition focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200'} />
              <input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="Dirección" className={isDark ? 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-600 bg-slate-900/70 p-3 text-sm text-white transition focus:border-emerald-500/60 focus:outline-none focus:ring-2 focus:ring-emerald-500/20' : 'ml-[3px] w-[calc(100%-3px)] rounded-xl border border-slate-300 bg-white p-3 text-sm text-slate-900 transition focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200'} />
            </div>
          </>
        )}
