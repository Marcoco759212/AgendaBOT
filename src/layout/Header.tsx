...
function Header() {
  const { tenants, activeTenantId, setActiveTenant, clearTenantSetupFlag, setTheme, theme, user, logout, setQuickCreateType } = useAppStore()
              ...
              {isTenantMenuOpen && (
                <div className={isDark ? 'absolute left-0 top-full z-30 mt-2 w-[220px] overflow-hidden rounded-xl border border-slate-600 bg-slate-900 shadow-[0_18px_45px_rgba(2,6,23,0.55)]' : 'absolute left-0 top-full z-30 mt-2 w-[220px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.12)]'}>
                  {tenants.map((tenant) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (tenant.needsSetup) {
                          clearTenantSetupFlag(tenant.id)
                    ...
                    >
                      ...
                      {tenant.needsSetup && (
                        <span className={isDark ? 'ml-2 shrink-0 rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-300' : 'ml-2 shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-700'}>
                          Nuevo
                        </span>
                      )}
                    </button>
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setQuickCreateType('branch')
                      setIsTenantMenuOpen(false)
                    }}
                    className={isDark
                      ? 'flex w-full items-center justify-between border-t border-slate-700 bg-slate-950/80 px-3 py-2.5 text-left text-sm font-medium text-emerald-200 transition-colors hover:bg-slate-800'
                      : 'flex w-full items-center justify-between border-t border-slate-200 bg-emerald-50 px-3 py-2.5 text-left text-sm font-medium text-emerald-700 transition-colors hover:bg-emerald-100'}
                  >
